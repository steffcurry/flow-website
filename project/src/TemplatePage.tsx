...
import { useParams } from "next/navigation"; // or replace with your router
...
export default function TemplatePage() {
  // If using Next.js App Router: const { id } = useParams<{ id: string }>();
  // If using Next.js Pages Router: const { id } = useRouter().query;
  // Fallback for standalone use:
  const params = useParams<{ id: string }>();
  const templateId = params?.id ?? "1";
      ...
      <div style={styles.card}>
        ...
        {invoiceUrl && (
          <>
            <div style={styles.divider} />
            <label style={styles.sectionLabel}>
              <CheckIcon color="#00c8ff" /> Invoice ready — complete your
              purchase
            </label>
