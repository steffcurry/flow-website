...
const T = {
  en: {
    ...
    minsPerHour: "min/h →",
    hoursYear: "hrs/year",
    pricingLabel: "Setup Cost (20% of ROI)",
  ...
  gr: {
    ...
    pricingLabel: "Setup Cost (20% του ROI)",
...
export default function ROICalculator() {
  ...
  const addQuickTask = useCallback(
    (qt: { task: string; mins: string }) => {
      ...
      // If the mandatory first bar is empty, fill it instead of adding a new row
      setTasks((prev) => {
        const firstEmpty = prev.findIndex((t) => !t.name.trim());
        if (firstEmpty !== -1) {
          return prev.map((t, i) =>
            i === firstEmpty ? { ...t, name: qt.task, minsPerHour: qt.mins } : t
          );
        }
        // All rows have names — add a new row
        setTaskCounter((c) => c + 1);
        return [...prev, { id: Date.now(), name: qt.task, minsPerHour: qt.mins }];
      });
    },
    [usedQuickTasks]
  );
  ...
  const calculateROI = useCallback(() => {
    ...
    // POST contact + ROI data to webhook
    ...
    const payload = {
      ...
      roi_analysis: {
        ...
        total_working_hours_year: Math.round(totalHoursYear),
        ...
        total_annual_roi_eur: Math.round(totalROI),
        system_cost_eur: Math.round(investmentCost),
      },
    ...
    fetch(WEBHOOK, {
      ...
      .then(() => saveFingerprint(fp))
      .catch(() => saveFingerprint(fp));
  }, [hoursDay, daysWeek, hourlyRate, tasks, contact, lang]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!results && (
          <>
            <div style={{ height: 1, background: css.border, margin: "4px 0 16px" }} />
            <Card>
              <div style={{ display: "flex", alignItems: "center", marginBottom: 8 }}>
                <StepDot n={3} />
                <span style={{ fontSize: 17, fontWeight: 700, color: css.text }}>{t.step3}</span>
              </div>
              <p style={{ fontSize: 13, color: css.text2, marginBottom: 22, lineHeight: 1.6 }}>
                {t.step3desc}
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14 }}>
                <FieldInput
                  label={t.firstName}
                  required
                  placeholder={lang === "en" ? "George" : "Γιώργος"}
                ...
                <FieldInput
                  label={t.lastName}
                  required
                  placeholder={lang === "en" ? "Papadopoulos" : "Παπαδόπουλος"}
                ...
                <FieldInput
                  ...
                  error={duplicateError ? t.errDuplicate : contactErrors.email}
                />
                <FieldInput
                  label={t.phone}
                  required
                ...
                <FieldInput
                  label={t.company}
                  required
                  placeholder={lang === "en" ? "My Business Ltd" : "Η Επιχείρησή μου ΑΕ"}
                ...
                <FieldSelect
                  ...
                  onChange={(e) => setContact((c) => ({ ...c, industry: e.target.value }))}
                >
        ...
        {/* ── Calculate Button ── */}
        {!results && (() => {
          const hasValidTask = tasks.filter((t) => t.name.trim() && parseFloat(t.minsPerHour) > 0).length > 0;
          ...
          const canCalculate = hasValidTask && hasValidContact;
          return (
            <button
              onClick={async () => {
                if (!canCalculate) {
              ...
              style={{
                width: "100%",
                padding: 16,
                background: canCalculate ? css.cyan : css.surface2,
                border: "none",
                borderRadius: 10,
                fontFamily: "inherit",
                fontSize: 16,
                fontWeight: 700,
                color: canCalculate ? "#0A0E1A" : css.text3,
